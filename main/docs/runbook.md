...
If the API is down the dashboard shows deterministic demo data and marks it.

## Build the dataset and run the evaluation

```bash
# offline, deterministic, zero downloads (demo and CI path)
python -m dgad.eval.datasets --build --synthetic

# train, calibrate, tune the gate on validation, run all 12 ablation configs
...
Outputs land in `results/`: `ablation.csv` (the report table),
`gate_tradeoff_curve.csv`, one `scores_<config>.csv` per configuration, and
`results/figures/*.png`. Real-data mode drops `--synthetic` and uses the
Hugging Face loaders (network required; see data/README.md for licenses).
...
| dashboard blank | API down | expected: it falls back to demo data with a banner |
...
- The behavioural probe and intent extractor need a local ollama model
