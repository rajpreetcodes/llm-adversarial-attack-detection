...
import { Alert, Button, Paper, Slider, Typography } from "@mui/material";
...
export default function ThresholdPlayground({
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(() => {
      ...
      postPlayground(t).then(setResult).catch(() => { setResult(null); setError(true); }).finally(() => setLoading(false));
    }, 250);
  ...
  return (
    <Paper className="reveal" sx={{ p: 2 }}>
      ...
      {loading && <div className="mt-2 h-14 animate-pulse border border-hairline bg-zinc-900" aria-label="Recomputing historical traffic" />}
      {error && <Alert severity="error" sx={{ mt: 2 }} action={<Button color="inherit" size="small" onClick={() => { setTD((v) => Math.min(1, v + 0.001)); }}>Retry</Button>}>Historical recomputation failed. No simulated metrics are shown.</Alert>}
      {!loading && !error && result?.n === 0 && <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>No historical prompts are available for threshold analysis.</Typography>}
    </Paper>
  );
}
