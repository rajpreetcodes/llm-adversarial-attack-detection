...
# Map reference numbers to working URLs (primarily arXiv links)
REFERENCE_URLS = {
    1: "https://arxiv.org/abs/2307.15043",
    2: "https://arxiv.org/abs/2211.09527",
    3: "https://arxiv.org/abs/2310.04451",
    4: "https://arxiv.org/abs/2410.21791",
    5: "https://arxiv.org/abs/2312.06674",
    6: "https://arxiv.org/abs/2310.12815",
    7: "https://arxiv.org/abs/2312.14197",
    8: "https://arxiv.org/abs/2308.14132",
    9: "https://arxiv.org/abs/2309.00614",
    10: "https://arxiv.org/abs/2502.15427",
    11: "https://arxiv.org/abs/2410.22770",
    12: "https://arxiv.org/abs/2402.13494",
    13: "https://arxiv.org/abs/2504.11168",
}
...
# Multimodal datasets (those that combine both text and image data in the same samples)
MULTIMODAL_DATASETS = [
    ("FigStep", "https://github.com/ThuCCSLab/FigStep",
     "Several hundred typographic-image jailbreak prompts",
     "Multimodal jailbreak set that hides harmful instructions inside innocuous-looking images, bypassing the text-only safety alignment of vision-language models."),
    ("MM-SafetyBench", "https://github.com/isXinLiu/MM-SafetyBench",
     "13 unsafe scenarios, each queried with stable-diffusion, OCR and typographic image variants",
     "Safety benchmark for multimodal LLMs in which the query-relevant image, not the accompanying text, carries the harmful payload."),
...
ROWS = [
    {
        ...
        "references": REFERENCE_URLS[1],  # Working link
    },
    {
        ...
        "references": REFERENCE_URLS[2],  # Working link
    },
    {
        ...
        "references": REFERENCE_URLS[3],  # Working link
    },
    {
        "author": "Su (2024)",
        ...
        "references": REFERENCE_URLS[4],  # Working link
    },
    {
        ...
        "references": REFERENCE_URLS[5],  # Working link
    },
    {
        "author": "Liu et al. (2023)",
        ...
        "references": REFERENCE_URLS[6],  # Working link
    },
    {
        ...
        "references": REFERENCE_URLS[7],  # Working link
    },
    {
        ...
        "references": REFERENCE_URLS[8],  # Working link
    },
    {
        ...
        "references": REFERENCE_URLS[9],  # Working link
    },
    {
        ...
        "references": REFERENCE_URLS[10],  # Working link
    },
    {
        ...
        "references": REFERENCE_URLS[11],  # Working link
    },
    {
        ...
        "references": REFERENCE_URLS[12],  # Working link
    },
    {
        ...
        "references": REFERENCE_URLS[13],  # Working link
    },
]
...
def build_excel(path: str) -> None:
    ...
    widths = [20, 30, 30, 44, 44, 20, 46, 42, 44, 30]  # Increased width for references column
...
def _write_dataset_sheet(wb, tab_name: str, banner_title: str, rows) -> None:
    ...
    widths = [30, 48, 40, 25, 25, 62]  # Dataset Name, Link, Size, Methods, References, Description
    ...
    for r, (name, link, size, desc) in enumerate(rows, start=3):
        # Determine methods based on dataset name/content
        methods = get_dataset_methods(name, desc)
        # For references, we'll use the dataset's own reference or a relevant one
        references = get_dataset_references(name, desc)
        ...
        for col, value in enumerate(values, start=1):
            ...
            cell.alignment = wrap_top
            cell.border = border
            if col == 2:  # Link column - make it clickable
                cell.hyperlink = link
            elif col == 5:  # References column - make it clickable if it's a URL
...
def get_dataset_methods(name: str, description: str) -> str:
    """Return specific methods/approach for each dataset"""
    name_lower = name.lower()
    
    if "advbench" in name_lower:
        return "Harmful behavior benchmark with GCG attack suffixes"
    elif "bipia" in name_lower:
        return "Indirect prompt injection across external content (email, web, tables, code)"
    elif "notinject" in name_lower:
        return "Benign prompts with injection-style trigger words for over-defence testing"
    elif "toxicchat" in name_lower:
        return "Real user-model conversations annotated for toxicity"
    elif "xstest" in name_lower:
        return "Exaggerated safety benchmark with safe/unsafe prompt contrasts"
    elif "anthropic" in name_lower or "hh-rlhf" in name_lower:
        return "Human preference dialogues for harmless vs harmful classification"
    elif "prompt injection benchmark" in name_lower or "liu et al" in name_lower:
        return "Formal framework pairing target tasks with injected tasks"
    elif "mnist" in name_lower:
        return "Grayscale digit classification (FGSM, PGD baseline)"
    elif "cifar-10" in name_lower:
        return "Color image classification across 10 classes (PGD, C&W baseline)"
    elif "imagenet" in name_lower and "ilsvrc" in name_lower:
        return "Large-scale image classification (transfer learning, robustness benchmark)"
    elif "nips 2017" in name_lower:
        return "Targeted adversarial examples for ImageNet (black-box evaluation)"
    elif "imagenet-a" in name_lower:
        return "Natural adversarial images (uncorrupted real-world photographs)"
    elif "robustbench" in name_lower:
        return "Standardised adversarial attack suites (FGSM, PGD, C&W, AutoAttack)"
    elif "figstep" in name_lower:
        return "Typographic-image jailbreak prompts (text embedded in images)"
    elif "mm-safetybench" in name_lower:
        return "Multimodal safety evaluation (image variants with stable-diffusion/OCR)"
    else:
        return "Standard adversarial benchmark dataset"


def get_dataset_references(name: str, description: str) -> str:
    """Return relevant reference links for each dataset"""
    name_lower = name.lower()
    
    # Map datasets to their key references
    if "advbench" in name_lower:
        return REFERENCE_URLS[1]  # Zou et al. 2023 (GCG)
    elif "bipia" in name_lower:
        return REFERENCE_URLS[7]  # Yi et al. 2023 (indirect injection)
    elif "notinject" in name_lower:
        return REFERENCE_URLS[11]  # Li and Liu 2024 (InjecGuard)
    elif "toxicchat" in name_lower:
        return REFERENCE_URLS[12]  # Xie et al. 2024 (GradSafe)
    elif "xstest" in name_lower:
        return REFERENCE_URLS[12]  # Xie et al. 2024 (GradSafe)
    elif "anthropic" in name_lower or "hh-rlhf" in name_lower:
        return REFERENCE_URLS[5]  # Inan et al. 2023 (Llama Guard)
    elif "prompt injection benchmark" in name_lower or "liu et al" in name_lower:
        return REFERENCE_URLS[6]  # Liu et al. 2023 (prompt injection formalization)
    elif "mnist" in name_lower:
        return "http://yann.lecun.com/exdb/mnist/"  # Original MNIST paper
    elif "cifar-10" in name_lower:
        return "https://www.cs.toronto.edu/~kriz/cifar.html"  # CIFAR-10 page
    elif "imagenet" in name_lower and "ilsvrc" in name_lower:
        return "https://www.image-net.org/"  # ImageNet website
    elif "nips 2017" in name_lower:
        return "https://github.com/google/nips-2017-adversarial-learning-dev-set"  # NIPS challenge
    elif "imagenet-a" in name_lower:
        return "https://github.com/hendrycks/natural-adv-examples"  # ImageNet-A
    elif "robustbench" in name_lower:
        return "https://github.com/RobustBench/RobustBench"  # RobustBench
    elif "figstep" in name_lower:
        return "https://github.com/ThuCCSLab/FigStep"  # FigStep
    elif "mm-safetybench" in name_lower:
        return "https://github.com/isXinLiu/MM-SafetyBench"  # MM-SafetyBench
    else:
        return "See dataset source for references"
