...
def test_real_notinject_loader_is_benign(monkeypatch):
    frames = {
        name: pd.DataFrame({"prompt": [f"benign {name} instruction"]})
        for name in ["NotInject_one", "NotInject_two", "NotInject_three"]
    }

    def fake_load(dataset, config=None, split="train"):
        assert dataset == "leolee99/NotInject"
        return frames[split]

    monkeypatch.setattr(ds, "_safe_hf_load", fake_load)
    result = ds._load_notinject()
    assert len(result) == 3
    assert set(result["source"]) == {"notinject"}
    assert set(result["label"]) == {0}
    assert set(result["attack_family"]) == {"none"}
    assert set(result["license"]) == {"MIT"}
