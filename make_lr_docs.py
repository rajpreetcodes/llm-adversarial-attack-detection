...
ROWS = [
    {
        ...
        "data": "AdvBench harmful behaviours and harmful strings; open-weight surrogates (Vicuna-7B/13B, LLaMA-2-7B-Chat); black-box targets ChatGPT, Bard and Claude via public interfaces. Synthetic attack generations.",
        "methodology": "Greedy Coordinate Gradient (GCG): a mix of greedy and gradient-based discrete token search that optimises a 20-token suffix on a white-box surrogate, extended to universal multi-prompt, multi-model suffixes; transfer measured on held-out models.",
    ...
    },
    {
        ...
        "data": "GPT-3 (davinci) few-shot prompts across standard NLP tasks; hand-crafted injection strings. Manually curated attack inputs.",
        "methodology": "Manual construction of injected instructions appended to user input; qualitative and quantitative evaluation of hijacking and prompt-leaking success across tasks.",
    ...
    },
    {
        ...
        "data": "Harmful behaviour sets derived from AdvBench; open-weight aligned LLMs as targets; optional warm start from existing manual jailbreak prompts. Synthetic attack generations.",
        "methodology": "Hierarchical genetic algorithm that evolves prompts at sentence level and then refines at word level, preserving readability while optimising jailbreak success.",
    ...
    },
    {
        ...
        "data": "Aligned open-weight LLMs; harmful behaviour sets; Llama Guard used as the safety classifier for evaluation. Synthetic attack generations.",
        "methodology": "CoT-GCG: GCG-style gradient optimisation aimed at reasoning-triggering prefixes rather than affirmative targets, evaluated with Llama Guard judgements.",
    ...
    },
    {
        ...
        "data": "Safety risk taxonomy with multiple harm categories; training on a mix of public red-teaming data (Anthropic hh-rlhf) and internal data. Labelled prompt-response pairs.",
        "methodology": "Instruction fine-tuning of LLaMA2-7B for safe/unsafe classification of inputs and outputs, with zero-shot and few-shot adaptation to new taxonomies and categories.",
    ...
    },
    {
        ...
        "data": "Constructed benchmark pairing target NLP tasks with injected tasks across multiple datasets. Synthetic benchmark of task pairs.",
        "methodology": "Formal framework modelling the attack as injected instruction plus data concatenated with the target instruction; systematic evaluation of prevention-based and detection-based defences under this framework.",
    ...
    },
    {
        ...
        "data": "BIPIA benchmark spanning five scenarios: email QA, web QA, table QA, summarisation and code QA. Constructed text tasks with embedded payloads.",
        "methodology": "Benchmark construction with indirect attacks placed in external content; evaluation of defences including delimiter-based prompt engineering and fine-tuning, split by white-box versus black-box access.",
    ...
    },
    {
        ...
        "data": "In-the-wild jailbreak prompts gathered from online collections, contrasted with benign prompts. Real-world collected attack prompts.",
        "methodology": "Score each prompt by perplexity under a reference LM and apply a threshold; for long prompts, compute perplexity over sliding windows and aggregate, so a short anomalous segment is not diluted.",
    ...
    },
    {
        ...
        "data": "AdvBench-style harmful behaviours; GCG and related attack variants; benign prompts for false-positive budgets. Synthetic attacks plus benign corpora.",
        "methodology": "Evaluation harness comparing perplexity filtering (including windowed), paraphrasing, retokenisation, LLM-based detection and adversarial training, each measured under fixed FPR budgets.",
    ...
    },
    {
        ...
        "data": "Broad collection of malicious jailbreak and injection datasets plus benign prompt sets; fifteen defences including Azure Prompt Shield and Meta Prompt Guard. Public benchmark corpora.",
        "methodology": "Unified benchmarking harness scoring each defence across attack families and benign sets, analysing score distributions and threshold behaviour rather than single accuracy numbers.",
    ...
    },
    {
        ...
        "data": "NotInject, a new benign set built to contain trigger words; existing prompt injection benchmarks. Curated benign and adversarial prompt corpora.",
        "methodology": "Fine-tuning encoder models (DeBERTa family) with the proposed MOF strategy to reduce over-defence; benchmark evaluation across injection sets and the NotInject benign set.",
    ...
    },
    {
        ...
        "data": "Jailbreak and benign prompt benchmarks including ToxicChat and XSTest; open-weight models for gradient access. Public benchmark corpora.",
        "methodology": "Pair each prompt with a compliant response prefix, backpropagate, slice gradients of safety-critical parameters; GradSafe-Zero uses cosine similarity to reference slices, GradSafe-Adapt trains a small logistic head.",
    ...
    },
    {
        ...
        "data": "Six guardrail systems including commercial offerings; established prompt injection and jailbreak attack corpora as base payloads. Public attack corpora plus live guardrail endpoints.",
        "methodology": "Apply character-injection and obfuscation techniques alongside adversarial machine learning evasion to attack payloads, and measure detection before and after transformation across the six systems.",
...
def build_excel(path: str) -> None:
    ...
    keys = ["author", "title", "background", "objective", "data",
            "methodology", "findings", "limitations", "relevance"]
    widths = [20, 34, 44, 44, 40, 44, 46, 42, 44]
...
def _write_dataset_sheet(wb, tab_name: str, banner_title: str, rows) -> None:
    ...
    ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=len(DATASET_HEADERS))
    ...
    for col, header in enumerate(DATASET_HEADERS, start=1):
    ...
    for r, (name, link, size, desc) in enumerate(rows, start=3):
        values = [name, link, size, desc]
        for col, value in enumerate(values, start=1):
            cell = ws.cell(row=r, column=col, value=value)
            cell.font = link_font if col == 2 else body_font
            cell.alignment = wrap_top
            cell.border = border
            if col == 2:
                cell.hyperlink = link
